...
class Headers implements \ArrayAccess, \IteratorAggregate
{
	 ...
	 * {@link Headers\ContentType} classes used to handle the values of the `Cache-Control`,
...
{
	/**
	 * The `disposition-type` part of the content disposition.
	 *
	 * @var string
	 */
	public $type;
	...
	/**
	 * If defined, the object is initialized with the content disposition.
	 *
	 * @param string $content_type
	 */
	public function __construct($content_disposition=null)
	{
		if ($content_disposition)
		{
			$this->set($content_disposition);
		}
	}

	/**
	 * Sets the content disposition, updating the properties of the object.
	 *
	 * @param string|array $content_disposition
	 */
	public function set($content_disposition)
	{
		...
		{
			preg_match('#(inline|attachment)(; filename="([^"]+)")?#', $content_disposition, $matches);

			if (isset($matches[1]))
			{
				$disposition_type = $matches[1];
			}

			if (isset($matches[3]))
			{
				$filename_param = $matches[3];
			}
		}

		if ($disposition_type != 'inline' && $disposition_type != 'attachment')
		{
			throw new \InvalidArgumentException('disposition-type must be "inline" or "attachment", given: "' . $disposition_type . '"');
		}

		$this->type = $disposition_type;
	...
	public function __toString()
	{
		...
		$filename_param = $this->filename;

		if ($filename_param)
		{
			$content_disposition .= '; ' . \ICanBoogie\HTTP\Headers::format_param('filename', $filename_param);
		}
